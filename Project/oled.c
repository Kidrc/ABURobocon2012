...
{	
	...
	for (i = 0; i < 8; ++i)
	{
		GPIO_ResetBits(OLED_SCLK);
		
		if (dat&0x80)
		{
			GPIO_SetBits(OLED_SDIN);
		}
		else
		{
			GPIO_ResetBits(OLED_SDIN);
		}
		GPIO_SetBits(OLED_SCLK);
...
*/
	...
	GPIO_SetBits(OLED_VCC);
	GPIO_ResetBits(OLED_GND);
	GPIO_SetBits(OLED_RST);
